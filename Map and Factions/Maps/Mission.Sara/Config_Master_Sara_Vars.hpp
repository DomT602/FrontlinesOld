...
AW_sectorSpawnRadius[] = {250,300,250,250,200}; //Distance units can spawn away from a sector centre in format {Town,City,Military,Radio,Factory}
...
AW_maxAmbientCivs = 5; //max ambient civs roaming around
AW_maxAmbientCivAir = 1; //max ambient civs flying around (only if map supports airports)
AW_maxAmbientOpforPatrols = 5; //max ambient enemy vehicles roaming around